...
// 推荐计算的入口，每一次推荐请求都会调用该方法
int Recommender::recommend(const ::std::string& user_id, ::std::string& item_ids_line) const {
    item_ids_line.clear();
    ...
    while (num < RECOMMEND_NUM) {
        ...
        if (diversity_filter(best_item, dedup_context)) {
            ...
            for (size_t i = pos + 1; i < top_items.size(); ++i) {
                ...
                if (diversity_filter(item, dedup_context)) {
                    continue;
                }
                // 找到修改最佳指针
                best_item = item;
    ...
    // 说明：记录每个推荐请求的分阶段耗时、中间结果等
    ::std::stringstream ss;     
    ss << now_time() << " user:" << user_id << " time_use:"
    ...
    << " history_size:" << dedup_context.history_items_num
    << " history_dedup_size:" << dedup_context.dedup_items_num
    << " index_result_size:" << top_items.size()
    << " history_filter_count:" << history_filter_count
    << " result_size:" << num
    << " result:" << item_ids_line;
...
bool Recommender::history_filter(Item* item, DedupContext& dedup_context) {
    if (!item || item == dedup_context.recent_item) {
        return true;
    }
