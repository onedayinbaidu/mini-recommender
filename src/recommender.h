...
public:
    ...
    // 运行时操作, 可重入的方法，必须是const
    int recommend(const ::std::string& user_id, ::std::string& item_ids_line) const;
...
private:
    ...
    bool history_filter(Item* item, DedupContext& dedup_context);
    ...
    std::size_t _index_candidate_num {256};
};
